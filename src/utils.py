...
def load_minimal_data(args):
    print("=====================")
    print("TODO: Load minimal data: A(adj matrix), Z(contexts/covariates), X(treatment), Y(outcome)")
    print ("================================Dataset================================")
    ...
    return trainA, trainX, trainT, POTrain
...
def PO_normalize_minimal(normy,base,PO):
    """Normalize PO"""
    if  normy:
        ym, ys = torch.mean(base), torch.std(base)
        YF = (PO - ym) / ys
...
#    pdist = torch.nn.PairwiseDistance(p=2)
    ...
    Mt = M
    row = delta*(torch.ones(M[0:1,:].shape).to(delta.device))
    col = torch.cat([delta*(torch.ones(M[:,0:1].shape).to(delta.device)),(torch.zeros((1,1))).to(delta.device)],0)
    if cuda:
        row = row.cuda()
        col = col.cuda()
