...
class GCN_DECONF_INTERFERENCE(nn.Module):
    def __init__(self, nfeat, nhid, dropout, n_in=1, n_out=1, cuda=True, num_motifs=None):
        ...
        if cuda:
            ...
            self.out_t00 = [nn.Linear(nhid+exp_dim,nhid).cuda() for i in range(n_out)]
            self.out_t10 = [nn.Linear(nhid+exp_dim,nhid).cuda() for i in range(n_out)]
            self.out_t01 = nn.Linear(nhid,1).cuda()
            self.out_t11 = nn.Linear(nhid,1).cuda()

        else:
            self.out_t00 = [nn.Linear(nhid+exp_dim,nhid) for i in range(n_out)]
            self.out_t10 = [nn.Linear(nhid+exp_dim,nhid) for i in range(n_out)]
            self.out_t01 = nn.Linear(nhid,1)
            self.out_t11 = nn.Linear(nhid,1)
    ...
    def forward(self, adj,x, t, Z=None,):
        ...
        if Z is None:
            neighbors = torch.sum(adj, 1)
            neighborAverageT = torch.div(torch.matmul(adj, t.reshape(-1)), neighbors)
            neighborAverageT = neighborAverageT.reshape(-1, 1)
        else:
            neighborAverageT = Z
        ...
        rep1 = torch.cat([rep, neighborAverageT], dim=1)
        for i in range(self.n_out):
